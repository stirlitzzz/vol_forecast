...
def build_har_features(vol_matrix: pd.DataFrame, term_1d=1, term_1w=5, term_1m=21) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def fit_har_regression(vol_matrix, har_1d, har_1w, har_1m, earnings_mask=None, min_obs=10):
    """
    ...
    for date in global_dates:
        if date not in vol_matrix.index or \
        date not in har_1d.index or \
        date not in har_1w.index or \
        date not in har_1m.index or \
        (earnings_mask is not None and date not in earnings_mask.index):
